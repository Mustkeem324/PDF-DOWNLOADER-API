...
def download_slide_images(url):
  ...
  if url.startswith("https://slideplayer.com"):
    ...
    # After creating the PDF, remove it from the current directory
    ...
    with ThreadPoolExecutor() as executor:
      futures = [
          executor.submit(download_image, f'https://{page["imageUri"]}')
          for page in response_data['document']['pages']
      ]
      for future in as_completed(futures):
        image_data_list.append(future.result())
    ...
    with ThreadPoolExecutor() as executor:
      futures = [
          executor.submit(download_image,
                          image.get("srcset").split(",")[-1].split("?")[0])
          for image in images
      ]
      for future in as_completed(futures):
        image_data_list.append(future.result())
